...
One-page емоційний сторителлінг-сайт-листівка до Дня Матері. 28 секцій вертикального скролу зі snap-snap між ними, кінематографічні переходи, фото, текстові моменти, фінальний Apple-style reveal Yakaboo-сертифіката з password gate.
...
## Контентна структура (28 секцій)

| # | Section / class | Mood | Тип | Що показує |
|---|---|---|---|---|
| 1 | `.act--intro` | dark | text | «Це був складний рік. Але сьогодні — твій день.» + scroll-hint |
...
| 5 | `.act--moment` | warm | text | «І навіть з такою мордою.» |
| 6 | `.act--photo .act--blanko` | warm | photo | Blanko смішна морда |
| 7 | `.act--moment` | warm | text | «А потім — у тобі прокинулося нове життя…» |
| 8 | `.act--moment` | warm | text | «Бланко був поряд. Спостерігав. Чекав.» |
| 9-10 | `.act--photo .act--blanko` | warm | photo | вагітна + Blanko (×2) |
| 11 | `.act--turn` | dark | special | paw trail (2 лапки + 2 baby foot) → typewriter «2025» → «02.07» → turn line |
| 12 | `.act--hero` | hero | photo | newborn full-bleed |
| 13 | `.act--moment` | bright | text | «Це був важкий рік.» |
| 14 | `.act--moment` | bright | text | «Безсонні ночі. Стрибки росту і крики о 3-й ранку.» |
| 15-16 | `.act--photo .act--maksym` | bright | photo | sleeping, mock-angry |
| 17 | `.act--moment` | bright | text | «Але ти продовжувала бути турботливою мамою.» |
| 18-20 | `.act--photo .act--maksym` | bright | photo | гойдалка, книжка, троє в ліжку |
| 21 | `.act--moment` | bright | text | «І ось — ця посмішка. Що ламає всі твої побоювання, сумніви і втому.» |
| 22 | `.act--photo .act--maksym` | bright | photo | зима |
| 23 | `.act--moment` | bright | text | «Вони завжди тебе вважатимуть мамою.» |
| 24 | `.act--photo .act--maksym` | bright | photo | Blanko + малий |
| 25 | `.act--moment .act--moment-final` | bright | text | «Ти неймовірна. І Максим це відчуває.» |
| 26 | `.act--hero .act--final-climax` | hero | photo | малий щасливий full-bleed |
| 27 | `.act--dedication` | warm | text | «Для найкращої мами на світі / Від: Максимчика, Бланчика і Олега» |
| 28 | `.act--gift` | cinematic | special | Apple reveal + Yakaboo cert + password gate |
...
### 4. Turn-секція (Act 11 — `.act--turn`)
Найскладніша анімація, послідовно:
1. Перехідний trail: 2 лапки Бланко + 2 дитячі ніжки (`.paw--baby`). Та сама `pawStep` keyframe з затримками 0.5s, 1.0s, 1.5s, 2.0s — стартують при `.is-visible`. Метафора: ера Бланко передає естафету ері малюка.
...
3. **Paw trail (mixed)** (`.paw-trail` з 4 SVG: 2 лапки Бланко + 2 `.paw--baby` дитячі ніжки) — у turn-секції перед роком. CSS-keyframe `pawStep` зі staggered animation-delays (0.5s, 1.0s, 1.5s, 2.0s). Свідомо асиметрично: лапки → ніжки = передача естафети.
...
### 10. Scroll-hint (Act 1)
`.scroll-hint` внизу intro-секції — підказка «проведи донизу» + chevron-стрілочка з infinite bounce-анімацією (`scrollBounce`, 1.8s). Сам блок fade-in через `scrollHintFade` з затримкою 1.6s — з'являється після того, як прочитався opening typewriter. `prefers-reduced-motion` вимикає bounce, лишає статичний chevron з opacity 0.85.
